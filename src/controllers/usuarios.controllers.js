...
export const crearUsuario = async (req, res) => {
  try {
    ...
    if (usuarioExistente) {
      return res
        .status(400)
        .json({ memsaje: "Este correo ya se encuentra registrado" });
    }
...
export const login = async (req, res) => {
  try {
    ...
    if (!usuarioExistente) {
      return res
        .status(400)
        .json({ memsaje: "Correo o pasasword incorrecto -email" });
    }
    ...
    if (!passwordValido) {
      return res
        .status(400)
        .json({ memsaje: "Correo o pasasword incorrecto -password" });
    }
    ...
    res
      .status(500)
      .json({ mensaje: "Ocurrio un error al intentar logiar un usuario" });
  }
};
